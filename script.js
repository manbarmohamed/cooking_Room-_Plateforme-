...
for (let i = 0; i < ArryRecipes.length; i++) {
    ...
    for (let j = 0; j < 5; j++) {
        if (j < ArryRecipes[i].rating) {
            starsHTML += `<i class="fas fa-star" data-index="${i}" data-rating="${j+1}" onclick="rateRecipe(this)"></i>`;
        } else {
            starsHTML += `<i class="far fa-star" data-index="${i}" data-rating="${j+1}" onclick="rateRecipe(this)"></i>`;
        }
    }
    ...
    <div class="col card-search" data-index="${i}">
        <div class="card d-flex justify-content-center align-items-center mt-5">
            ...
            <div class="card-body d-flex justify-content-center align-items-center flex-column">
                ...
                <div class="rating">${starsHTML}</div> <!-- Rating stars -->
...
function rateRecipe(star) {
    const recipeIndex = parseInt(star.getAttribute('data-index'));
    const rating = parseInt(star.getAttribute('data-rating'));
    ArryRecipes[recipeIndex].rating = rating;
    getCard()
}
// **************************************
